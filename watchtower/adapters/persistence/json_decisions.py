...
The decision engine depends on the :class:`~watchtower.ports.stores.DecisionStore`
protocol, never on a concrete backend. :class:`JsonDecisionStore` is the initial
local implementation. The store keeps current decisions, an append-only event
log, and the reviews made over time; prior reasoning is never overwritten.
"""
